...
const TABLE_COLORS = {
  Approved: {
    bg: "rgba(42,92,63,0.3)",
    border: "rgba(76,175,122,0.4)",
    text: "#4caf7a",
  },
  Seated: {
    bg: "rgba(42,92,63,0.3)",
  ...
  Reserved: {
    bg: "rgba(196,137,42,0.15)",
    border: "rgba(196,137,42,0.3)",
    text: T.adminAmber,
  },
  Incoming: {
    bg: "rgba(196,137,42,0.15)",
...
// Display label shown on the table map card — distinct from the raw
// reservation status (which still shows in the table's Badge column).
// Approved/Seated tables read as "Occupied" here; everything else keeps
// its status text (Pending, Reserved, etc.), Free stays Free.
const TABLE_DISPLAY_LABEL = {
  Approved: "Occupied",
  Seated: "Occupied",
};
...
export default function Reservations() {
  ...
  const updateStatus = async (id, status, extra = {}) => {
    ...
    try {
      ...
      serverErrorMessage = err.response?.data?.message || "Update failed";
    }

    if (!serverErrorMessage) {
      // Refresh immediately from the server instead of trusting a local
      // optimistic patch — guarantees the table/status the UI shows is
      // exactly what's persisted, and picks up any server-side side effects
      // (e.g. another reservation auto-freed, table reassignment, etc.)
      await fetchReservations(selectedDate);
      showToast(TOAST[status] || "Updated!");
      return;
    }

    // Reconcile with the server: re-fetch and check whether the change
    // actually landed despite the error response / network hiccup.
    try {
      await fetchReservations(selectedDate);

      let fresh;
      setReservations((prev) => {
        fresh = prev.find((r) => r._id === id);
        return prev; // no-op update, just used to peek at latest state
      });

      const matchesExpected =
        fresh?.status === status &&
        Object.keys(extra).every((key) => fresh?.[key] === extra[key]);
      ...
      // ignore, fall through to failure toast below
    }
  ...
  const openConfirm = (reservation) => {
    ...
    const hasValidTable =
      TABLES.includes(reservation.table) && !taken.includes(reservation.table);
  ...
  const deleteReservation = async () => {
    try {
      ...
      // Immediate refresh from server rather than a local filter, so the
      // list reflects the real current state right away.
      await fetchReservations(selectedDate);
  ...
  const openEdit = (reservation) => {
    ...
    setEditForm({
      ...
      table: TABLES.includes(reservation.table) ? reservation.table : "T-01",
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        // Preserve the existing status explicitly — some backends do a
        // full-document update rather than a partial merge, so omitting
        // this field can wipe out status and break the Badge / table map.
  ...
  const add = async () => {
    ...
    if (newRes) {
      // Immediate refresh — if the new reservation's date matches
      // selectedDate it'll show up right away instead of waiting on the
      // socket event.
      await fetchReservations(selectedDate);
  ...
  const tableStatusMap = TABLES.map((t) => {
    const res = safeReservations.find(
      (r) =>
        r &&
        r.table === t &&
        !FREE_STATUSES.includes(r.status) &&
  ...
  return (
    <div className="fade-up">
      <StatsGrid>
        ...
        <StatCard
          ...
          value={
            safeReservations.filter(
              (r) => r?.status === "Approved" || r?.status === "Seated",
            ).length
          }
          valueColor="#4caf7a"
        />
        ...
        <StatCard
          label="Total Guests"
          value={safeReservations.reduce(
            (a, r) => a + (Number(r?.guests) || 0),
            0,
          )}
        />
      </StatsGrid>
      ...
      <Panel
        ...
        action={
          <div style={{ display: "flex", gap: 10 }}>
            <input
              type="date"
              style={{ ...inputStyle(), width: 160, padding: "6px 12px" }}
              value={selectedDate}
        ...
        >
          {sortedReservations.map((r) => (
            <tr key={r._id || r.id}>
              ...
              <Td mono>{r.guests}</Td>
              <Td mono>{TABLES.includes(r.table) ? r.table : "—"}</Td>
              <Td mono>{r.phone}</Td>
              <Td>
                <Badge type={BADGE[r.status] || "wait"}>{r.status}</Badge>
              </Td>
              <Td>
      ...
      <Panel title="Table Map">
        ...
        >
          {tableStatusMap.map((t) => {
            ...
            const label = TABLE_DISPLAY_LABEL[t.status] || t.status;
            return (
              <div
                key={t.id}
                onClick={() => {
                ...
                style={{
                  ...
                  padding: "12px 8px",
                  textAlign: "center",
                  cursor: t.status === "Free" ? "default" : "pointer",
                }}
              ...
              >
                ...
                <div
                  style={{ fontSize: "0.68rem", marginTop: 4, color: c.text }}
                >
                  {label}
                </div>
              </div>
        ...
        {confirmTarget && (
          <>
            ...
            >
              ...
              <strong style={{ color: "rgba(245,240,232,0.95)" }}>
            ...
            <FormGroup label="Assign Table">
              ...
              >
                {TABLES.map((t) => {
                  ...
                  const isOccupied = occupiedTables(
                    resDate,
                    confirmTarget._id,
                  ).includes(t);
                  return (
      ...
      >
        ...
        >
          ...
          <FormGroup label="Table">
            <select
              style={selectStyle()}
              value={editForm.table || "T-01"}
              onChange={(e) =>
                setEditForm((p) => ({ ...p, table: e.target.value }))
              }
            >
              {TABLES.map((t) => {
                const isOccupied = occupiedTables(
        ...
        >
          ...
          <FormGroup label="Table">
            <select
              ...
              value={form.table}
              onChange={(e) =>
                setForm((p) => ({ ...p, table: e.target.value }))
              }
            >
              {TABLES.map((t) => {
      ...
      >
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
          ...
          <Btn variant="danger" onClick={deleteReservation}>
            Yes, Delete
          </Btn>
        </div>
      </Modal>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
          <Btn variant="ghost" onClick={() => setCancelId(null)}>
            No, Keep It
          </Btn>
          <Btn
            variant="danger"
